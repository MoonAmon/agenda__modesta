...
function weeklyCalendar() {
  ...
  return {
    ...
    init: function () {
      ...
      // Auto-refresh: recarregar eventos a cada 15 segundos
      var self = this;
      ...
      fetch("/agenda/api/week/?week_start=" + wsISO, {
        ...
        .then(function (data) {
          ...
          console.log(
            ...
            wsISO,
            "| Eventos recebidos:",
            self.rawEvents.length,
            self.rawEvents,
          );
          self._processEvents();
          console.log(
            "[Calendário] Eventos processados:",
            self.processedEvents.length,
    ...
    _processEvents: function () {
      ...
      self.rawEvents.forEach(function (ev, idx) {
        ...
        var durationSlots = Math.max(
          1,
          Math.round(durationMs / (30 * 60 * 1000)),
        );
    ...
    getEventsForCell: function (dayIdx, hour, half) {
      return this.processedEvents.filter(function (ev) {
        return (
          ev.day === dayIdx && ev.startHour === hour && ev.startHalf === half
        );
      });
    },
  };
